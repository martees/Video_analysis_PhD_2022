...
def visit_versus_previous_travel(results, curve_list, travel_time_bins, only_first):
    for i_curve, curve in enumerate(curve_list):
        ...
        if len(curve) == 1:
            variable_value_bins, average_visit_duration, [errors_inf, errors_sup] = plots.plot_visit_time(results, [],
                                                                                                          "",
                                                                                                          curve,
                                                                                                          "last_travel_time",
    ...
    if only_first == 1:
        ...
        plt.ylabel("Average duration of first visit to a patch (minutes, log scale)", fontsize=12)
    elif only_first is not False:
        plt.title("Average of first " + str(only_first) + " visit length, OD=" + param.nb_to_density[curve[0]])
        plt.ylabel("Average duration of first " + str(only_first) + " visits to a patch (minutes, log scale)", fontsize=12)
    else:
        plt.title("Average of all visits as a function of the duration of the previous transit")
        plt.ylabel("Average duration of all visits (minutes, log scale)", fontsize=12)
    ...
    plt.xlabel("Duration of the previous travel (hours, log scale)", fontsize=12)
    plt.xscale("log")
    plt.yscale("log")
    plt.ylim(10, 6800)
    plt.show()
...
visit_versus_previous_travel(clean_results, [[param.name_to_nb["close 0"]], [param.name_to_nb["med 0"]], [param.name_to_nb["far 0"]],
             [param.name_to_nb["superfar 0"]]], travel_time_bins=bin_list, only_first=only_first_visit)
visit_versus_previous_travel(clean_results, [[param.name_to_nb["close 0.2"]], [param.name_to_nb["med 0.2"]], [param.name_to_nb["far 0.2"]],
             [param.name_to_nb["superfar 0.2"]]], travel_time_bins=bin_list, only_first=only_first_visit)
visit_versus_previous_travel(clean_results, [[param.name_to_nb["close 0.5"]], [param.name_to_nb["med 0.5"]], [param.name_to_nb["far 0.5"]],
             [param.name_to_nb["superfar 0.5"]]], travel_time_bins=bin_list, only_first=only_first_visit)
visit_versus_previous_travel(clean_results, [[param.name_to_nb["close 1.25"]], [param.name_to_nb["med 1.25"]], [param.name_to_nb["far 1.25"]],
             [param.name_to_nb["superfar 1.25"]]], travel_time_bins=bin_list, only_first=only_first_visit)
