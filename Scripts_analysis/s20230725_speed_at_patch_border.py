...
def one_condition(clean_trajectories, clean_results, condition_list, condition_name,
                  time_window, axs, show_only_density):
    ...
    for i_folder in range(len(folder_list)):
        ...
        for i_entry in range(len(entry_time_stamps)):
            ...
            if (post_entry_index - pre_entry_index)*param.one_frame_in_seconds > (2*time_window)-1:
        ...
        for i_exit in range(len(exit_time_stamps)):
            ...
            if (post_exit_index - pre_exit_index) * param.one_frame_in_seconds > (2 * time_window) - 1:
    ...
    if show_only_density:
        color = param.name_to_color[param.nb_to_density[param.name_to_nb[condition_name]]]
        label = "OD = "+param.nb_to_density[param.name_to_nb[condition_name]]
    else:
...
def condition_pool(condition_names, show_only_density=False):
    ...
    axs[0].set_title("Entry", fontsize=24)
    axs[0].set_ylabel("Centroid speed (mm/s)", fontsize=16)
    axs[0].set_xlabel("Time pre/post entry (s)", fontsize=16)
    axs[0].set_ylim(0.03, 0.26)
    axs[1].set_title("Exit", fontsize=24)
    axs[1].set_xlabel("Time pre/post exit (s)", fontsize=16)
    axs[1].set_ylim(0.03, 0.26)

    for i_condition, condition in enumerate(condition_list):
        print("Condition ", condition_names[i_condition], " (", i_condition,"/",len(condition_list),")")
        one_condition(clean_trajectories, clean_results, condition,
                      condition_names[i_condition], time_window, axs, show_only_density)

    # convert the x axis to seconds
    ticks = axs[0].get_xticks()
    ticks = ticks[1::2]  # first is negative for some reason, exclude it
    seconds = [i * param.one_frame_in_seconds for i in ticks]
    axs[0].set_xticks(ticks, [str(np.round(i,1)) for i in seconds])
