...
is_recompute_full_data_table = False
...
for condition in condition_list:
    ...
    if is_recompute_full_data_table:
        ...
        # We create a matrix to store the data, with each line corresponding to a time in patch, each column to a
        ...
        for i_folder in range(len(current_folder_list)):
            ...
            if is_recompute_pixelwise_delays == True:
                visits_to_pixels_matrix = pixel_wise_leaving_delay(current_folder_list[i_folder])
            else:
...
for i_condition in range(len(list_of_matrices)):
    matrix_this_condition = list_of_matrices[i_condition]
